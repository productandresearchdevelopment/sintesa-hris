  ...
  var OrganizationsDetail = function() {
    ...
    me.init = function() {
      me.store = Ext.create('Ext.data.TreeStore', {
        ...
        proxy: {
          type: 'ajax',
          url: '{{ route('helpdesk.category.data.organization', '') }}/',
        },
        listeners: {
          load: function() {
    ...
    me.setAuth = function() {
      ...
      if (rec) {
        http.request({
          method: 'post',
          url: '{{ route('helpdesk.category.set.organization', '') }}/' + helpdesk.id,
          params: {
            organization: rec.id,
    ...
    me.storeLoad = function() {
      setTimeout(function() {
        ...
        if (rec) {
          ...
          me.store.proxy.url = '{{ route('helpdesk.category.data.organization', '') }}/' + rec.id
          me.store.load();
        }
      }, 500);
    }
