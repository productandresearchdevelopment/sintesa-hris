  ...
  var OrganizationsDetail = function() {
    ...
    me.init = function() {
      me.store = Ext.create('Ext.data.TreeStore', {
        id: 'organizations-store-tree-bulletin',
        fields: [{
            name: 'id',
            type: 'int'
          },
        ...
        proxy: {
          type: 'ajax',
          url: '{{ route('bulletin.category.data.organization', '') }}/'
        },
        listeners: {
          load: function() {
            const grid = me.grid || Ext.ComponentQuery.query('#grid-organizations-bulletin')[0];
      ...
      me.grid = Ext.create('Ext.tree.Panel', {
        id: 'grid-organizations-bulletin',
        region: 'center',
        title: 'ORGANIZATIONS',
        rootVisible: false,
        singleExpand: true,
    ...
    me.storeLoad = function() {
      setTimeout(function() {
        ...
        if (rec) {
          ...
          me.store.proxy.url = '{{ route('bulletin.category.data.organization', '') }}/' + rec.id;
          me.store.load();
        }
      }, 500);
    };
